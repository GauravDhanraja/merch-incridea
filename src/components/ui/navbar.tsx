    ...
    <nav className="sticky top-0 z-50 w-screen border-b border-neutral-700/80 bg-neutral-900 py-3 backdrop-blur-lg">
      <div className="container relative mx-auto px-4 lg:text-sm">
        <div className="flex items-center justify-between">
          ...
          {/* Desktop Nav */}
          <ul className="hidden lg:flex items-center space-x-12 text-xl font-bold text-white">
            {navItems.map((item) => (
              <li key={item.href}>
          ...
          <div className="hidden lg:flex items-center space-x-4">
            ...
            {session ? (
              <button
                className="rounded-md border border-white px-3 py-2 text-white hover:bg-white hover:text-black font-bold"
                onClick={() => signOut()}
              >
                Sign Out
              </button>
