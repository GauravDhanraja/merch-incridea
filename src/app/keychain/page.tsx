...
function Home() {
  ...
  useEffect(() => {
    if (merchData) {
      setPrice(merchData[2].discountPrice); // Assuming the API returns an array with a `price` field
      setName(merchData[2].name);
      setDescription(merchData[2].description);
      setTotalCount(merchData[2].stock);
      setPriceWithFanumTax(merchData[2].originalPrice);
      setImageLink(merchData[2].image);
    }
  }, [merchData]);
